...
function Senderdetails() {
    ...
    const fetchOrder = async () => {
      if (orderId) {
        try {
          ...
          // If there's a senderId, fetch the sender details
          if (order.data.senderId) {
            const sender = await axios.get(`/api/senders/${order.data.senderId}`)
            if (sender.data) {
              setSenderDetails(sender.data)
              // Reset form with sender data
              form.reset({
                ...sender.data,
                bankCharges: sender.data.bankCharges || "resident",
                occupationStatus: sender.data.occupationStatus || "employed",
                sourceOfFunds: sender.data.sourceOfFunds || "salary",
                relationship: sender.data.relationship || payer,
              })
            }
          } else {
            // No sender exists, populate with order data for student details
            if (order.data.studentName) {
              form.setValue("studentName", order.data.studentName)
            }

            // For "self" payer, populate sender details with student details
            if (payer === "self") {
              form.setValue("senderName", order.data.studentName || "")
  ...
  useEffect(() => {
    ...
    // For "self" payer, auto-populate sender name with student name
    if (payer === "self") {
      const studentName = form.getValues("studentName")
      if (studentName && !form.getValues("senderName")) {
        form.setValue("senderName", studentName)
      }
    }
  }, [payer, form])

  // Watch for changes in student name to update sender name for "self" payer
  useEffect(() => {
    if (payer === "self") {
      const subscription = form.watch((value, { name }) => {
        if (name === "studentName" && value.studentName) {
          form.setValue("senderName", value.studentName)
        }
      })
              ...
              <CardContent className="p-0 sm:p-0">
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 sm:space-y-8">
                    ...
                    {/* Sender Details Section - Always shown now, but fields change based on payer */}
                            ...
                            <FormItem className="space-y-1 sm:space-y-2">
                              ...
                              <Select onValueChange={field.onChange} value={field.value}>
                                ...
                                <SelectContent className="rounded-none">
                                  <SelectItem value="self" className="rounded-none">
                                    Self
                                  </SelectItem>
