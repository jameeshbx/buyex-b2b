...
import { S3FileUploader } from "@/app/(protected)/staff/(components)/s3-file-uploader";
...
export default function DocumentUploadForm({
  ...
  const [senderDetails] = useState<Sender | null>(null);
  const [orderId] = useState<string | null>(orderID);
  const [currentUserState] = useState(currentUser);
  const [existingDocuments] = useState<ExistingDocument[]>([]);
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      if (orderId) {
        try {
          const order = await axios.get(`/api/order-public/${orderId}`);
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            !(
              reuploading?.section === "kyc" && reuploading?.field === "pan"
            ) ? (
              <div className="p-3 bg-green-50 border border-green-200 rounded flex items-center justify-between">
                ...
                <Button
                  ...
                  className="ml-2"
                  onClick={() =>
                    setReuploading({ section: "kyc", field: "pan" })
                  }
                >
                  Re-upload
                </Button>
          ...
          <div>
            ...
            !(
              reuploading?.section === "kyc" &&
              reuploading?.field === "identity"
            ) ? (
              <div className="p-3 bg-green-50 border border-green-200 rounded flex items-center justify-between">
                <div>
                ...
                <Button
                  ...
                  className="ml-2"
                  onClick={() =>
                    setReuploading({ section: "kyc", field: "identity" })
                  }
                >
                  Re-upload
                </Button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {purpose &&
            CHECKLIST_FIELDS[purpose]?.map((item) => {
              ...
              const isReuploading =
                reuploading?.section === "checklist" &&
                reuploading?.field === item.type;
              return (
                <div key={item.type}>
          ...
          {educationLoan === "yes" && (
            <div>
              ...
              !(
                reuploading?.section === "checklist" &&
                reuploading?.field === "LOAN_SANCTION_LETTER"
              ) ? (
                <div className="p-3 bg-green-50 border border-green-200 rounded flex items-center justify-between">
                  <div>
                    <p className="text-green-800 text-sm">
                    ...
                    <a
                      href={
                        formState.checklist.LOAN_SANCTION_LETTERUrl as string
                      }
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-blue-600 hover:text-blue-800 text-xs"
